...
class Analyzer:
  ...
  def output_all_job_info(self, agg_results_filename):
    file_writer = csv.writer(open("data.csv", "wb"))
    file_writer.writerow(['job_id','num_stages',\
                            'Fraction time scheduler delay', \
                            'Fraction time waiting on shuffle read',\
                            'Speedup from eliminating disk for input', \
                            'Speedup from elimnating disk for output', \
                            'Speedup from eliminating disk for shuffle write', \
                            'Speedup from eliminating shuffle read', \
                            'No disk speedup', \
                            'No network speedup', \
                            'Fraction of time spent writing/reading shuffle data to/from disk', \
                            'Fraction of time spent garbage collecting', \
                            'Speedup from eliminating compute', \
                            'Fraction of time waiting on compute', \
                            'Fraction of time computing', \
                            'Speedup from eliminating stragglers: perfect parallelsim', \
                            'Speedup from eliminating stragglers: use average', \
                            'Speedup from eliminating stragglers: use median', \
                            'Speedup from eliminating stragglers: use (1.5=>median)', \
                            'Speedup from eliminating stragglers: use (95%%ile=>med)', \
                            'median progress rate', \
                            'Simulated runtime', \
                            'Actual runtime', \
                            'Simulated versus actual runtime'
                            ])

    for job_id, job in self.jobs.iteritems():
      filename = "%s_%s" % (self.filename, job_id)
      row = self.__output_job_info(job, job_id, filename, agg_results_filename)
  ...
  def __output_job_info(self, job, job_id, filename, agg_results_filename):
    ...
    data_row = [job_id, len(job.stages.keys())]

    fraction_time_scheduler_delay = job.fraction_time_scheduler_delay()
    data_row.append(fraction_time_scheduler_delay)
    ...
    data_row.append(fraction_time_waiting_on_shuffle_read)
    ...
    data_row.append(no_shuffle_write_disk_speedup)
    ...
    data_row.append(no_shuffle_read_disk_speedup)
    ...
    data_row.append(no_disk_speedup);
    ...
    data_row.append(fraction_time_waiting_on_compute)
    ...
    data_row.append(fraction_time_computing);
    ...
    data_row.extend([no_stragglers_perfect_parallelism, replace_all_tasks_with_average_speedup, \
      no_stragglers_replace_with_median_speedup, no_stragglers_replace_ganesh_with_median_speedup, \
      no_stragglers_replace_95_with_median_speedup, median_progress_rate_speedup])

    simulated_versus_actual, simulated_runtime, actual_runtime = job.simulated_runtime_over_actual(filename)
    data_row.extend([simulated_runtime, actual_runtime, simulated_versus_actual])
